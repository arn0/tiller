...
enum commands {COMMAND_CODE=0xc7, RESET_CODE=0xe7, MAX_CURRENT_CODE=0x1e, MAX_CONTROLLER_TEMP_CODE=0xa4, MAX_MOTOR_TEMP_CODE=0x5a, RUDDER_RANGE_CODE=0xb6, RUDDER_MIN_CODE=0x2b,
					RUDDER_MAX_CODE=0x4d, REPROGRAM_CODE=0x19, DISENGAGE_CODE=0x68, MAX_SLEW_CODE=0x71, EEPROM_READ_CODE=0x91, EEPROM_WRITE_CODE=0x53, CLUTCH_PWM_AND_BRAKE_CODE=0x36};
...
enum {SYNC=1, OVERTEMP_FAULT=2, OVERCURRENT_FAULT=4, ENGAGED=8, INVALID=16, PORT_PIN_FAULT=32, STARBOARD_PIN_FAULT=64, BADVOLTAGE_FAULT=128, MIN_RUDDER_FAULT=256,
		MAX_RUDDER_FAULT=512, CURRENT_RANGE=1024, BAD_FUSES=2048, /* PORT_FAULT=4096  STARBOARD_FAULT=8192 */ REBOOTED=32768};


struct struct_control_settings { uint16_t low_current;
											uint16_t max_current;
											uint16_t max_controller_temp;
											uint16_t max_motor_temp;
											uint16_t rudder_min;
											uint16_t rudder_max;
											uint16_t max_slew_speed;
											uint16_t max_slew_slow;
											uint8_t clutch_pwm;
...
void process_packet( pypi_packet packet )
{
	...
	switch( packet.p.command ) {
		...
		case RESET_CODE:
			ESP_LOGI( TAG, "Received RESET_CODE, value %d", value );			// reset overcurrent flag
		...
		case COMMAND_CODE:
			...
			if( value > 2000 );															// unused range, invalid!!!
